...
function createButtons(path, dis) {
  ...
  if (path === posts) {
    ...
    path.forEach((element) => {
      ...
      option.text = element.replace('.json', '');
...
async function getData(item, file, page = 1) {
  ...
  try {
    ...
    if (!data[itemName]) {
      throw new Error(`Data for item "${itemName}" not found.`);
    }

    const start = (page - 1) * itemsPerPage;
    const end = start + itemsPerPage;
    const paginatedData = data[itemName].slice(start, end);
    ...
    newObj.forEach((element) => {
      ...
      const languageClass = languageClassMapping[itemName] || "language-none";
      div.innerHTML += `
              <h1>${element.title}</h1>
    ...
    updatePaginationControls(item, page, data[itemName].length, file);
  } catch (error) {
...
function updatePaginationControls(item, page, totalItems, file) {
  paginationControls.innerHTML = "";
  if (page > 1) {
    const prevButton = document.createElement("button");
    prevButton.innerText = "Previous";
    prevButton.onclick = () => getData(item, file, page - 1);
    paginationControls.appendChild(prevButton);
  }
  if (page * itemsPerPage < totalItems) {
    const nextButton = document.createElement("button");
    nextButton.innerText = "Next";
    nextButton.onclick = () => getData(item, file, page + 1);
